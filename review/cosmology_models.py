...
class LCDM(object):
    ...
    def hubble_normalized_z(self, z):
        ...
        """
        return np.sqrt(self.Om0 * (1. + z)**3. + self.Or0 *
                       (1 + z)**4 + (1. - self.Om0 - self.Or0))
    ...
    def z_decoupling(self):
        ...
        return 1048 * (1. + 0.00124 * (self.Ob0 * self.h**2) **
                       (-0.738)) * (1 + g1 * (self.Om0 * self.h**2)**g2)
    ...
    def z_drag_epoch(self):
        ...
        return 1291. * (self.Om0 * self.h**2)**0.251 * (1 + b1 * (self.Om0 * \
                        self.h**2)**b2) / (1 + 0.659 * (self.Om0 * self.h**2)**0.828)
    ...
    def sound_horizon(self, z):
        ...
        R1 = 31500 * (self.Ob0 * self.h**2) * \
            (2.728 / 2.7)**(-4) * 1 / (1. + z)

        def integrand1(a):
            return 1 / (np.sqrt(3 * (1 + R1)) *
                        self.hubble_normalized_a(a) * a**2)
    ...
    def cmb_shift_parameter(self):
        ...
        Output:
          R (shift parameter)
        """
        return np.sqrt(self.Omega_m_z(0.)) * (1 + self.z_decoupling()) * \
            self.angular_diameter_distance_z(self.z_decoupling()) / self.D_H()
    ...
    def acoustic_length(self):
        ...
        return np.pi * \
            self.angular_diameter_distance_z(z_star) * (1 + z_star) / self.sound_horizon(z_star)
    ...
    def deriv(self, y, a):
        return [y[1], -
                (3. /
                 a +
                 self.hubble_prime_normalized_a(a) /
                 self.hubble_normalized_a(a)) *
                y[1] +
                1.5 *
                self.Omega_m_a(a) *
                y[0] /
...
class wCDM(LCDM):
    ...
    def hubble_normalized_z(self, z):
        return np.sqrt(self.Om0 * (1. + z)**3. + self.Or0 * (1 + z) **
                       4 + (1. - self.Om0 - self.Or0) * (1. + z)**(3 * (1 + self.w)))
...
class w0waCDM(LCDM):
    ...
    def hubble_normalized_z(self, z):
        return np.sqrt(self.Om0 * (1. + z)**3. + self.Or0 * (1 + z)**4 + (1. - self.Om0 - self.Or0)
                       * (1. + z)**(3 * (1 + self.w0 + self.wa)) * np.exp(-3. * self.wa * (z / (1. + z))))
...
class w0waCDMzi(LCDM):
    ...
    def hubble_normalized_z(self, z):
        return np.sqrt(self.Om0 * (1. + z)**3. + self.Or0 * (1 + z)**4 + (1. - self.Om0) * (1. + z)
                       ** (3 * (1 + self.w0 + self.wa / (1 + self.zi))) * np.exp(-3. * self.wa * (z / (1. + z))))
...
class GCG(LCDM):
    ...
    def hubble_normalized_z(self, z):
        return np.sqrt(self.Om0 * (1. + z)**3. + self.Or0 * (1 + z)**4 + (1. - self.Om0 - self.Or0)
                       * (self.As + (1 - self.As) * (1 + z)**(3 * (1 + self.alpha)))**(1 / (1 + self.alpha)))
