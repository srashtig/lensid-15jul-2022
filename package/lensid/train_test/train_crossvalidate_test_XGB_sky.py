...
def main():
    parser = argparse.ArgumentParser(
        description='This is stand alone code for training, testing and cross_validating XGBoost with Skymaps.')
    parser.add_argument(
        '-df_dir_train',
        '--df_dir_train',
        help='input train Dataframe with sky feautures path',
        default='/home/srashti.goyal/strong-lensing-ml/data/dataframes/train/')
    parser.add_argument(
        '-df_dir_test',
        '--df_dir_test',
        help='input test Dataframe with sky features path',
...
def _main(odir,df_dir_train,df_dir_test,tag, train_size_lensed, cv_size_lensed, scale_pos_weight, max_depth, n_estimators, cv_splits, compare_to_blu, path_to_blu):
    ...
    df_unlensed_sky_half = pd.read_csv(
        ...
        '.csv',
        index_col=[0])
    df_unlensed_sky_half = df_unlensed_sky_half.sample(
        frac=1, random_state=42).reset_index(drop=1)
    df_train_sky = pd.concat(
        [df_lensed_sky, df_unlensed_sky_half], ignore_index=1)
    ...
    df_unlensed_sky_half = pd.read_csv(
        ...
        '.csv',
        index_col=[0])
    df_unlensed_sky_second_half = pd.read_csv(
        df_dir_train +
        'unlensed_second_half_sky' +
        tag +
        '.csv',
    ...
    for i, (train_index, test_index) in enumerate(
        ...
        viz = plot_roc_curve(
            xgboost_sky_model,
            X,
            df_cv_sky.Lensing.values[test_index],
    ...
    for i in range(1, cv_splits +1):
        ...
        df_test_sky['xgb_pred_bayestar_skymaps_' +
                    str(i)] = df['xgb_pred_bayestar_skymaps']

        false_positive_rate, true_positive_rate, thresholds = ml.roc_curve(
            df_test_sky.Lensing.values, df_test_sky['xgb_pred_bayestar_skymaps_' + str(i)])
    ...
    std_auc = np.std(aucs)
    ax.plot(
        mean_fpr, mean_tpr, label=r'Mean ROC  ML Skymaps (AUC = %0.2f $\pm$ %0.3f)' %
    ...
    for i, col in enumerate(cols):
        false_positive_rate, true_positive_rate, thresholds = ml.roc_curve(
            df_test_sky.Lensing.values, df_test_sky[col])
        plt.plot(
            false_positive_rate,
            true_positive_rate,
            '-',
    ...
    if compare_to_blu == 1:
        ...
        fig, rocs = ml.plot_ROCs(
            df_test_sky, cols=[
                'xgb_pred_bayestar_skymaps', 'bayestar_skymaps_blu', 'ra, sin_dec'], labels=[
                'ML skymaps', '$B^L_U$ Bayestar skymaps', '$B^L_U$ : RA DEC'], logy=1, ylim=1e-2)
        ...
        plt.savefig(
            odir +
            '/plots' +
            '/compare-thresholds-blu-XGB_sky' +
            tag +
            '.png')
        plt.show()
