...
def main():
    parser = argparse.ArgumentParser(
        description='This is stand alone code for generating QTs for lensed injection dataset')
    ...
    parser.add_argument(
        '-infile',
        '--infile',
        help='.npz lensed injs file path to load tags from',
        required=True)
    parser.add_argument(
        '-psd_mode',
        '--psd_mode',
    ...
    parser.add_argument(
        ...
        '--qrange',
        type=int,
        help='1. default is 3,7 for m1>60 and 4,10 otherwise. 2. wide is 3,30',
        default=1)
    ...
    for count, i in enumerate(ids[args.start:args.start + args.n]):
        ...
        for j in range(0, 2):
            hp, hc = get_td_waveform(approximant="IMRPhenomPv2", mass1=data["m1"][i], mass2=data['m2'][i], inclination=data[
                                     "incl"][i], delta_t=1.0 / 2**12, f_lower=15, f_higher=1000, distance=data["dist"][i][j], coa_phase=data['phi0'][i])
            det_h1 = Detector("H1")
            det_l1 = Detector("L1")
            det_v1 = Detector("V1")
            ...
            noise_signal_h1 = qtils.inject_noise_signal(
                signal_h1, psd_H, duration=duration, whitened=whitened)
            noise_signal_l1 = qtils.inject_noise_signal(
                signal_l1, psd_L, duration=duration, whitened=whitened)
            noise_signal_v1 = qtils.inject_noise_signal(
                signal_v1, psd_V, duration=duration, whitened=whitened)

            fname = str(data['event_tag'][i]) + '_' + \
                str(data['img_tag'][i, j])
            if whitened == 1:
                fname = fname + '-whitened'
            pow_H1[count, j] = qtils.plot_qt_from_ts(
                noise_signal_h1, data["t0"][i][j], q, outfname=odir + "/H1/" + fname)
            pow_L1[count, j] = qtils.plot_qt_from_ts(
                noise_signal_l1, data["t0"][i][j], q, outfname=odir + "/L1/" + fname)
            pow_V1[count, j] = qtils.plot_qt_from_ts(
                noise_signal_v1, data["t0"][i][j], q, outfname=odir + "/V1/" + fname)
