...
def main():
    ...
    for count, i in enumerate(range(args.start, args.start + args.n)):
        if args.qrange == 1:
            ...
            q = qtils.q_wide
        hp, hc = get_td_waveform(approximant="IMRPhenomPv2", mass1=data["m1z"][i], mass2=data['m2z'][i], inclination=data["iota"][
                                 i], delta_t=1.0 / 2**12, f_lower=15, f_higher=1000, distance=data["ldistance"][i], coa_phase=data['phi0'][i])
        ...
        pow_H1[count] = qtils.plot_qt_from_ts(
            noise_signal_h1, data["tc"][i], q, outfname=odir + "/H1/" + fname)
        pow_L1[count] = qtils.plot_qt_from_ts(
            noise_signal_l1, data["tc"][i], q, outfname=odir + "/L1/" + fname)
        pow_V1[count] = qtils.plot_qt_from_ts(
            noise_signal_v1, data["tc"][i], q, outfname=odir + "/V1/" + fname)
        fnames[count] = fname
