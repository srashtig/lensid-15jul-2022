...
def main():
    parser = argparse.ArgumentParser(
        description='This is stand alone code for testing combined ML and comparing to blu')
    parser.add_argument(
        '-indir',
        '--indir',
        help='input Dataframe directory',
        default='out/dataframes')
    parser.add_argument(
        '-odir',
        '--odir',
    ...
    parser.add_argument(
        '-tag_qts',
        '--tag_qts',
        help='tag for reading ML with qtransforms predictions ',
        default='_kaggle')
    parser.add_argument(
        '-tag_sky',
        '--tag_sky',
        help='tag for reading MK with skymaps predictions ',
        default='')
...
def _main(odir,indir, tag_sky, tag_qts, cv_splits, compare_to_blu):
    ...
    df_test['densnet_xgbsky_bayestar_mul_0'] = df_test['xgb_dense_QTS_0'] * \
        df_test['xgb_pred_bayestar_skymaps']
    ...
    for i in range(1, cv_splits+1):
        ...
        false_positive_rate, true_positive_rate, thresholds = ml.roc_curve(
            df_test.Lensing.values, df_test['densnet_xgbsky_bayestar_mul_' + str(i)])
    ...
    print(
        r'Mean ROC  ML combined (AUC = %0.3f $\pm$ %0.3f)' %
        (mean_auc, std_auc))
    ...
    if compare_to_blu == 1:
        ...
        for i, col in enumerate(cols):
            false_positive_rate, true_positive_rate, thresholds = ml.roc_curve(
                df_test.Lensing.values, df_test[col])
            plt.plot(
                false_positive_rate,
                true_positive_rate,
                '-',
    ...
    if compare_to_blu == 1:
        ...
        fig, rocs = ml.plot_ROCs(
            df_test, cols=[
                'densnet_xgbsky_bayestar_mul_0', 'm1, m2, ra, sin_dec'], labels=[
                'ML bayestar sky x ML QTs', '$B^L_U$'], logy=1, ylim=7e-2)
        ...
        plt.savefig(
            odir +
            '/plots' +
            '/compare-thresholds-blu-ML_combined' +
            tag_qts +
            tag_sky +
            '.png')
