...
for i, merger in enumerate(mergers_gwtc2):
    ...
    for det in ['H1', 'L1', 'V1']:
        ...
        if os.path.isfile(fname):
            ...
            hdata_1 = pycbc.types.TimeSeries(
                strain,
                delta_t=times[1] -
                times[0],
                epoch=times[0],
            ...
            times_h1, freqs_h1, power_h1 = hdata_whitened.qtransform(
                1 / 1024, logfsteps=100, qrange=q, frange=(15, 1000))
            # extra thing added to adjust the clim in presence of noise!
            st, end = np.where(
                times_h1 <= gps - 0.2)[0][-1], np.where(times_h1 >= gps + 0.1)[0][0]

            pylab.figure(figsize=[7, 4])
            pylab.pcolormesh(times_h1[st:end],
                             freqs_h1,
                             power_h1[:,
                                      st:end]**0.5 / np.max(power_h1[:,
                                                                     st:end]**0.5),
                             cmap="viridis")
            ...
            times_h1, freqs_h1, power_h1 = hdata_1.qtransform(
                1 / 1024, logfsteps=100, qrange=q, frange=(15, 1000))
            st, end = np.where(
                times_h1 <= gps - 0.2)[0][-1], np.where(times_h1 >= gps + 0.1)[0][0]
