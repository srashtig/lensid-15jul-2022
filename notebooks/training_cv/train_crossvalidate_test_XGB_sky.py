...
# train
df_lensed_sky = pd.read_csv(df_dir_train+'lensed_sky'+tag+'.csv',index_col=[0] )[:train_size_lensed]
...
xgboost_sky_model=train_xgboost_sky(df_train_sky,scale_pos_weight=scale_pos_weight)

if not os.path.exists(odir):
        os.makedirs(odir)


joblib_file = odir+'/XGBsky_0'+tag+'.pkl'  
...
plt.savefig(odir+'/validate-ROC-XGB_sky'+tag+'.png')


# # cross-validate XGB sky

df_lensed_sky = pd.read_csv(df_dir_train+'lensed_sky'+tag+'.csv',index_col=[0] )[:cv_size_lensed]
df_unlensed_sky_half = pd.read_csv(df_dir_train+'unlensed_half_sky'+tag+'.csv' ,index_col=[0])
...
for i,(train_index, test_index) in enumerate(cv.split(df_cv_sky,df_cv_sky.Lensing.values)):
    xgboost_sky_model=train_xgboost_sky(df_cv_sky.iloc[train_index],scale_pos_weight=scale_pos_weight)
    joblib_file = odir+"/XGBsky_"+str(i+1)+".pkl"  
    joblib.dump(xgboost_sky_model, joblib_file)
...
plt.grid()
plt.savefig(odir+'/cv-ROC-XGB_sky'+tag+'.png')
plt.show()
...
xgboost_sky_model = joblib.load(odir+'/XGBsky_0'+tag+'.pkl')
...
for i in range(1,11):
    xgb_sky_cv = joblib.load(odir+"/XGBsky_"+str(i)+".pkl")
...
ax.grid()
ax.legend(loc ="lower right")
plt.savefig(odir+'/test-ROC-log-sky-xgb'+tag+'.png')
plt.show()

df_test_sky.to_csv(odir+'/ML_sky.csv')
...
plt.legend()
plt.savefig(odir+'/compare-blu-XGB_sky'+tag+'.png')
...
plt.savefig(odir+'/compare-FPPs-blu-XGB_sky'+tag+'.png')
plt.show()
...
plt.xlabel('FPP')
plt.ylabel('Threshold')
plt.savefig(odir+'/compare-thresholds-blu-XGB_sky'+tag+'.png')
plt.show()

df_test_qts.to_csv(odir+'/ML_sky'+tag+'.csv')