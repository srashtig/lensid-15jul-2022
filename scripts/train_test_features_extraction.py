...
def main():
    ...
    if cal_features_qts == 1:
        ...
        for df in train_dfs_dict.keys() :
            print('train ', df)
            os.system('nohup lensid_get_features_qts_ml -infile %s -outfile %s -data_dir %s -dense_models_dir %s -whitened %d -n %d >%s.out &'%(df_dir_train+df + '.csv',(base_out_dir+df_dir_train_features_out+df+'_qts'+tag_qts_out+'.csv'),data_dir_qts_train, dense_model_dir_in, whitened,train_dfs_dict[df], base_out_dir+'/train_'+df+'_qts'+tag_qts_out ))
        for df in test_dfs_dict.keys() :
            print('test ', df)
