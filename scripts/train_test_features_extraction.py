...
def main():
    ...
    if calc_features_sky == 1:
        ...
        for df in train_dfs_dict.keys():
            print('train ', df)
            os.system(
                'nohup lensid_get_features_sky_ml -infile %s -outfile %s -data_dir %s  -n %d >%s.out &' %
                (df_dir_train +
                 df +
                 '.csv',
                 (base_out_dir +
                  df_dir_train_features_out +
                  df +
                  '_sky' +
                  tag_sky_out +
                  '.csv'),
    ...
    if cal_features_qts == 1:
        ...
        for df in train_dfs_dict.keys():
            print('train ', df)
            os.system(
                'nohup lensid_get_features_qts_ml -infile %s -outfile %s -data_dir %s -dense_models_dir %s -whitened %d -n %d >%s.out &' %
                (df_dir_train +
                 df +
                 '.csv',
                 (base_out_dir +
