...
# optional: Get False Positive Probabilities using distribution of ML statictic with background injections 

def main():
    parser = argparse.ArgumentParser(description='This is stand alone code for computing ML predictions for a given event pairs in dataframe, their skymaps, Qtransforms and the trained ML models. Optionally computes the False Positive Probabilities given the background.')
    ...
    df_combined[col_dict['ML combined']] = df_combined[col_dict['ML QTs']]*df_combined[col_dict['ML sky']]

    plt.figure(figsize=(20,5))
    bins=np.linspace(-11,0,30)
    plt.subplot(131)
    ml_stat= col_dict['ML combined']
    plt.hist(np.log10(df_combined[ml_stat]),bins=bins,label='ML combined', histtype='step',density=True,lw=2,ls='dashed')
    plt.legend()
    plt.ylabel('PDF')
    plt.xlabel('ML Statistic')
    plt.subplot(132)
    ml_stat= col_dict['ML sky']
    plt.hist(np.log10(df_combined[ml_stat]),bins=bins,label='ML sky ', histtype='step',density=True,lw=2,ls='dashed')
    plt.legend()
    plt.ylabel('PDF')
    plt.xlabel('ML Statistic')
    ...
    plt.hist(np.log10(df_combined[ml_stat]),bins=bins,label='ML qts ', histtype='step',density=True,lw=2,ls='dashed')
    plt.legend()
    plt.ylabel('PDF')
    plt.xlabel('ML Statistic')
    ...
    if calc_fpp == 1:
        ...
        for stat in fpp_dict.keys():
            df_combined[col_dict[stat]+'_fpp']=ml.get_fars(df_combined,col_dict[stat],df_injs,fpp_dict[stat])

        plt.figure(figsize=(7,5))
        bins=np.linspace(-5,0,20)

        ml_stat= col_dict['ML combined']
        plt.hist(np.log10(df_combined[ml_stat+'_fpp']),bins=bins,label='ML combined', histtype='step',density=False,lw=2,ls='dashed')

        ml_stat= col_dict['ML sky']
        plt.hist(np.log10(df_combined[ml_stat+'_fpp']),bins=bins,label='ML sky ', histtype='step',density=False,lw=2,ls='dashed')

        ml_stat= col_dict['ML QTs']
        plt.hist(np.log10(df_combined[ml_stat+'_fpp']),bins=bins,label='ML qts ', histtype='step',density=False,lw=2,ls='dashed')
        ...
        plt.figure(figsize=(7,5))
        plt.scatter(df_combined[col_dict['ML QTs'] + '_fpp'],df_combined[col_dict['ML sky'] + '_fpp'], c=df_combined[col_dict['ML combined']+'_fpp'],norm=matplotlib.colors.LogNorm())
        plt.xscale('log')
        plt.yscale('log')
