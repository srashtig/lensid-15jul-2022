...
def main():
    parser = argparse.ArgumentParser(
        description='This is stand alone code for training, testing and cross_validating XGBoost with Qtransforms.')
    parser.add_argument(
        '-df_dir_train',
        '--df_dir_train',
        help='input train Dataframe with QT feautures path',
        default='/home/srashti.goyal/strong-lensing-ml/data/dataframes/train/')
    parser.add_argument(
        '-df_dir_test',
        '--df_dir_test',
        help='input test Dataframe with QT feautures path',
        default='/home/srashti.goyal/strong-lensing-ml/data/dataframes/test/')
    parser.add_argument(
        '-odir',
        '--odir',
        help='Output directory to save models dataframes and plots',
        default='out')
    parser.add_argument(
        '-tag',
        '--tag',
        help='Tag for the dataframes',
        default='_kaggle')
    ...
    parser.add_argument(
        '-path_to_blu',
        '--path_to_blu',
        help='help path to haris et al blu directory',
        default='/home/srashti.goyal/strong-lensing-ml/data/dataframes/haris_et_al/')
    parser.add_argument(
        '-train_size_lensed',
        '--train_size_lensed',
        type=int,
        help='no. of lensed pairs to train on',
        default=2400)
    parser.add_argument(
        '-cv_size_lensed',
        '--cv_size_lensed',
        type=int,
        help='no. of lensed pairs to crossvalidate on',
        default=2400)
    parser.add_argument(
        '-cv_splits',
        '--cv_splits',
    ...
    parser.add_argument(
        ...
        '--scale_pos_weight',
        type=float,
        help='scale_pos_weight of XGBoost algorithm',
        default=0.01)
    ...
    parser.add_argument(
        ...
        '--max_depth',
        type=int,
        help='max_depth of XGBoost algorithm',
        default=6)
    ...
    parser.add_argument(
        ...
        '--n_estimators',
        type=int,
        help='n_estimators of XGBoost algorithm',
        default=135)
...
def _main(odir,df_dir_train,df_dir_test,tag, train_size_lensed, cv_size_lensed, scale_pos_weight, max_depth, n_estimators, cv_splits, compare_to_blu, path_to_blu ):
    ...
    df_unlensed_qts_half = pd.read_csv(
        ...
        '.csv',
        index_col=[0])
    df_unlensed_qts_half = df_unlensed_qts_half.sample(
        frac=1, random_state=42).reset_index(drop=1)
    df_train_qts = pd.concat(
        [df_lensed_qts, df_unlensed_qts_half], ignore_index=1)
    ...
    df_unlensed_qts_half = df_unlensed_qts_half.sample(
        frac=1, random_state=42).reset_index(drop=1)
    df_val_qts = pd.concat(
        [df_lensed_qts, df_unlensed_qts_half], ignore_index=1)
    df_val_qts = df_val_qts.sample(frac=1).reset_index(drop=1)
    df_val_qts = ml.predict_xgboost_dense_qts(
        df_val_qts, xgboost_dense_qts_model)
    ...
    df_unlensed_qts_half = pd.read_csv(
        ...
        '.csv',
        index_col=[0])
    df_unlensed_qts_second_half = pd.read_csv(
        df_dir_train +
        'unlensed_second_half_QTs' +
        tag +
        '.csv',
    ...
    for i, (train_index, test_index) in enumerate(
            cv.split(df_cv_qts, df_cv_qts.Lensing.values)):
        xgboost_dense_qts_model = ml.train_xgboost_dense_qts(
        ...
        viz = plot_roc_curve(
            xgboost_dense_qts_model,
            X,
            df_cv_qts.Lensing.values[test_index],
            name="ROC fold {}".format(
                i + 1),
            alpha=0.3,
            lw=1,
            ax=ax)
    ...
    xgboost_dense_qts_model = joblib.load(
        odir + '/models/XGBQT_0' + tag + '.pkl')

    df_test_qts = ml.predict_xgboost_dense_qts(
        df_test_qts, xgboost_dense_qts_model)
    ...
    for i in range(1, cv_splits +1):
        ...
        false_positive_rate, true_positive_rate, thresholds = ml.roc_curve(
            df_test_qts.Lensing.values, df_test_qts['xgb_dense_QTS_' + str(i)])
    ...
    ax.plot(
        mean_fpr,
        mean_tpr,
        label=r'Mean ROC Combined H1 L1 V1 QTs ML (AUC = %0.2f $\pm$ %0.2f)' %
        (mean_auc,
         std_auc),
        lw=2,
        alpha=.8)
    ...
    for i, col in enumerate(cols):
        false_positive_rate, true_positive_rate, thresholds = ml.roc_curve(
            df_test_qts.Lensing.values, df_test_qts[col])
        plt.plot(
            false_positive_rate,
            true_positive_rate,
            '-',
    ...
    if compare_to_blu == 1:
        ...
        blu_stat = 'm1, m2'
        df_test[ml_stat +
                '_fpp'] = ml.get_fars(df_test, ml_stat, df_test, ml_stat)
        df_test[blu_stat +
                '_fpp'] = ml.get_fars(df_test, blu_stat, df_test, blu_stat)
        ...
        # plt.ylim(-100,1e3)
        df = df_test[df_test['Lensing'] == 0]
        plt.hist(np.log10(df[ml_stat + '_fpp']),
                 bins=bins,
                 label='ML Unlensed',
                 histtype='step',
                 density=1,
                 color='C0',
                 lw=4)
        plt.hist(np.log10(df[blu_stat + '_fpp']),
                 bins=bins,
                 label='BLU Unlensed',
                 histtype='step',
                 density=1,
        ...
        plt.subplot(122)
        df = df_test[df_test['Lensing'] == 0]
        plt.loglog(df[ml_stat + '_fpp'], df[blu_stat + '_fpp'],
                   '+', label='Unlensed', color='C0')
        df = df_test[df_test['Lensing'] == 1]
        plt.loglog(df[ml_stat + '_fpp'], df[blu_stat + '_fpp'],
                   'x', label='Lensed', color='C1')
        ...
        plt.savefig(
            odir +
            '/plots' +
            '/compare-FPPs-blu-XGB_QT' +
            tag +
            '.png')
        plt.show()

        fig, rocs = ml.plot_ROCs(
            df_test_qts, cols=[
                'dense_H1_0', 'dense_L1_0', 'dense_V1_0', 'xgb_dense_QTS_0', 'm1, m2'], labels=[
                'ML H1 QTs', 'ML L1 QTs', 'ML V1 QTs', 'ML combined H1 L1 V1 QTS', '$B^L_U:$ $m_1 m_2$'], logy=1)
