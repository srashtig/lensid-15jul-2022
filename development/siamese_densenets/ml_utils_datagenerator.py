...
class DataGenerator(Sequence):
    ...
    def __init__(self, df, labels, batch_size=32, dim=(128,128,3), n_channels=3,
                 n_classes=2,det,
    ...
    def __getitem__(self, index):
        ...
        # Find list of IDs
        df = [self.df.iloc[k] for k in indexes]

        # Generate data
        X, y = self.__data_generation(df)
...
class generate_resize_siamese_fm:
    """
    Class to generate input feature matrix to the DenseNets using Qtransform
        images.
    ...
    def Siamese_input_matrix(
        ...
        missing_ids = np.union1d(missing_ids_0, missing_ids_1).astype(int)
        if phenom == 0:
            return x_0,x_1, labels, missing_ids
        else:
            return x_0,x_1, labels, missing_ids, df